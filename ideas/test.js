	...
	stroke : function(f, c) {
		...
		for (e = 0; e < this.points.length; e++) {
			...
			if (g < 1000) {
				...
				this.context.beginPath();
				this.context.moveTo(this.points[this.count][0] + (b * 0.2),
						this.points[this.count][1] + (a * 0.2));
	...
	stroke : function(g, c) {
		...
		for (f = 0; f < this.points.length; f++) {
			...
			if (h < 4000 && Math.random() > h / 4000) {
				this.context.beginPath();
				this.context.moveTo(this.points[this.count][0] + (b * e),
						this.points[this.count][1] + (a * e));
				this.context.lineTo(this.points[f][0] - (b * e) + Math.random()
						* 2, this.points[f][1] - (a * e) + Math.random() * 2);
				this.context.stroke()
			}
		}
		this.count++
...
ribbon.prototype = {
	...
	update : function() {
		...
		for (a = 0; a < this.painters.length; a++) {
			...
			this.painters[a].dx -= this.painters[a].ax = (this.painters[a].ax + (this.painters[a].dx - this.mouseX)
					* this.painters[a].div)
					* this.painters[a].ease;
			this.painters[a].dy -= this.painters[a].ay = (this.painters[a].ay + (this.painters[a].dy - this.mouseY)
					* this.painters[a].div)
					* this.painters[a].ease;
	...
	stroke : function(f, c) {
		...
		for (e = 0; e < this.points.length; e++) {
			...
			if (g < 4000 && Math.random() > g / 2000) {
				this.context.beginPath();
				this.context.moveTo(this.points[this.count][0] + (b * 0.3),
...
ColorSelector.prototype = {
	...
	updateLuminosity : function(g) {
		var d, e, l, b, a, k = 100, f = 120, h, j = 1080 / 2, c = Math.PI / 180;
		...
		for (h = 0; h < j; h++) {
			e = h / (j / 360) * c;
			l = 255 - (h / j) * 255;
			d.strokeStyle = "rgb(" + Math.floor(g[0] - l) + ","
					+ Math.floor(g[1] - l) + "," + Math.floor(g[2] - l) + ")";
			d.beginPath();
			d.moveTo(Math.cos(e) * k + b, Math.sin(e) * k + a);
			d.lineTo(Math.cos(e) * f + b, Math.sin(e) * f + a);
			d.stroke()
		}
		this.luminosityData = d.getImageData(0, 0, this.luminosity.width,
				this.luminosity.height).data
	},
	update : function(g) {
		var b, k, f, c, h, a, j;
		...
		if (h < 90) {
			...
			this.updateLuminosity( [ this.hueData[(b + (k * 250)) * 4],
					this.hueData[(b + (k * 250)) * 4 + 1],
					this.hueData[(b + (k * 250)) * 4 + 2] ])
		} else {
			if (h > 100 && h < 120) {
				a = f / h;
				j = c / h;
	...
	getColor : function() {
		...
		return [ this.luminosityData[(a + (b * 250)) * 4],
				this.luminosityData[(a + (b * 250)) * 4 + 1],
				this.luminosityData[(a + (b * 250)) * 4 + 2] ]
	}
};
function Palette() {
	var f, e, b, a, q = 0, h = 90, o = 1080 / 2, n = 30, m, d = Math.PI / 180, l, k, p, g, r;
	...
	for (l = 0; l < o; l++) {
		...
		for (k = 0; k < n; k++) {
			...
			e.beginPath();
			e.moveTo(Math.cos(g) * (m * k + q) + b, Math.sin(g) * (m * k + q)
					+ a);
			e.lineTo(Math.cos(g) * (m * (k + 1) + q) + b, Math.sin(g)
					* (m * (k + 1) + q) + a);
			e.stroke()
		}
	}
...
Menu.prototype = {
	...
	setBackgroundColor : function(a) {
		...
		b.fillStyle = "rgb(" + a[0] + ", " + a[1] + ", " + a[2] + ")";
		b.fillRect(0, 0, this.backgroundColor.width,
...
var i, style, STYLES = [ "sketchy", "shaded", "chrome", "fur", "longfur",
		"web", "", "simple", "squares", "ribbon", "", "circles", "grid" ], COLOR = [
		0, 0, 0 ], BACKGROUND_COLOR = [ 250, 250, 250 ], SCREEN_WIDTH = window.innerWidth, SCREEN_HEIGHT = window.innerHeight, container, foregroundColorSelector, backgroundColorSelector, menu, about, canvas, flattenCanvas, context, mouseX = 0, mouseY = 0, isForegroundColorSelectorVisible = false, isBackgroundColorSelectorVisible = false, isAboutVisible = false;
isForegroundColorSelectorMouseDown = false,
		isBackgroundColorSelectorMouseDown = false, isMenuMouseOver = false,
		isMouseDown = false, controlKeyIsDown = false;
init();
function init() {
	var hash, palette;
	...
	menu.foregroundColor
			.addEventListener("click", onMenuForegroundColor, false);
	menu.backgroundColor
			.addEventListener("click", onMenuBackgroundColor, false);
...
function onWindowResize(a) {
	...
	menu.container.style.left = ((SCREEN_WIDTH - menu.container.offsetWidth) / 2)
			+ "px";
	about.container.style.left = ((SCREEN_WIDTH - about.container.offsetWidth) / 2)
			+ "px";
	about.container.style.top = ((SCREEN_HEIGHT - about.container.offsetHeight) / 2)
			+ "px"
}
...
function onMenuBackgroundColor(a) {
	...
	backgroundColorSelector.container.style.left = ((SCREEN_WIDTH - backgroundColorSelector.container.offsetWidth) / 2)
			+ "px";
	backgroundColorSelector.container.style.top = ((SCREEN_HEIGHT - backgroundColorSelector.container.offsetHeight) / 2)
			+ "px";
