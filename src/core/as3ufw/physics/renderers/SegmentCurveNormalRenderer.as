package as3ufw.physics.renderers {
	...
	public class SegmentCurveNormalRenderer extends GraphicsRenderer {
		public function SegmentCurveNormalRenderer(graphics : Graphics, brushParams : BrushParams) {
		...
		override public function render(g : ParticleGroup) : void {
			...
			while (particle) {
				...
				var points : Vector.<Vector2D> = CurveUtils.InterpolateQuadraticBezier(new Vector2D((particle.oldPos.x + particle.prevPos.x) * 0.5, (particle.oldPos.y + particle.prevPos.y) * 0.5), new Vector2D(particle.prevPos.x, particle.prevPos.y), new Vector2D((particle.pos.x + particle.prevPos.x) * 0.5, (particle.pos.y + particle.prevPos.y) * 0.5), 5);
				var normal : Vector2D;
