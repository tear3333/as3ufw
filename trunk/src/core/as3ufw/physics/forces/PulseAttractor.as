package as3ufw.physics.forces {
	 ...
	 */
	public class PulseAttractor extends AbstractForce implements IForceGenerator {

		private var forcePosition : Vector2D;
		private var strength : Number;

		public function PulseAttractor(forcePosition : Vector2D,strength : Number) {
		...
		override public function applyForce(targetParticle : Particle) : void {
			...
			forceVector.multEquals(strength * Math.cos(targetParticle.count));
			//targetParticle.addForce(forceVector);
			targetParticle.skew(forceVector);
			trace(forceVector);
		}
	}
}
